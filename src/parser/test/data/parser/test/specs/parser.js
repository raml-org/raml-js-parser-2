...
////    describe('feature flags', function (){
        ...
        it('should not validate title if flag is set not to validate 2', function(done) {
            var definition = [
                '#%RAML 0.8',
                '---',
                'title: ["title", "title line 2", "title line 3"]',
        ...
        it('should not apply transformations if flag is set to ignore 0', function(done) {
            var definition = [
                '#%RAML 0.8',
                '---',
                'title: some title',
        ...
        it('should not apply transformations if flag is set to ignore 1', function(done) {
            var definition = [
                '#%RAML 0.8',
                '---',
                'title: some title',
        ...
        it('should apply transformations if flag is set to ignore 2', function(done) {
            var definition = [
                '#%RAML 0.8',
                '---',
                'title: some title',
        ...
        it('should apply transformations if flag is set to ignore 3', function(done) {
            var definition = [
                '#%RAML 0.8',
                '---',
                'title: some title',
...
//
    it('should fail if there is a root property with wrong displayName', function(done) {
      ...
      raml.load(definition).should.be.rejectedWith(/Unknown node: 'wrongPropertyName'/).and.notify(done);
    });
//
...
//
    it('should fail if there is a root property with array', function(done) {
      ...
      raml.load(definition).should.be.rejectedWith(/Unknown node: /).and.notify(done);
    });
////  });
////
////  describe('Include', function() {

    //FIXTEST changed error message with the equal one from the new parser.
    it('should fail if include not found', function(done) {
      ...
      raml.load(definition).should.be.rejectedWith(/Can not resolve relative\.md/).and.notify(done);
    });
//
...
//        '#%RAML 0.8',
//        '---',
//        '!include '+__dirname+"/../raml-files/external.raml"
//      ].join('\n');
//
...
////  describe('URI Parameters', function() {
   ...
   //FIXTEST (this is not logical and does not specified anywhere)
    it('should pass when a parameter uses array syntax with only one type', function(done) {
      var definition = [
        '#%RAML 0.8',
        '---',
        'title: Test',
  ...
  //FIXTEST (it is not allowed to use file in baseUriParameters)
    ...
    //FIXTEST changed message from /Unused url parameter/
    it('should fail when declaring a URI parameter not on the baseUri', function(done) {
      ...
      raml.load(definition).should.be.rejectedWith("Base uri parameter unused").and.notify(done);
    });
...
//
    it('should fail when declaring an enum with duplicated values', function(done) {
      ...
      raml.load(definition).should.be.rejectedWith(/'enum' value contains repeating components/).and.notify(done);
    });
//
    ...
    //FIXTEST it was written that it should be rejected
...
//
    ...
    //FIXTEST files are only allowed in forms
    it('should fail when declaring a URI parameter with a file type', function(done) {
      var definition = [
        '#%RAML 0.8',
        '---',
        'title: Test',
      ...
      raml.load(definition).should.be.rejectedWith(/File type can be only used in web forms/).and.notify(done);;
    });
//
...
//      FIXTEST no reason why it should not be allowed
      it('should pass when a parameter uses array syntax with only one type', function(done) {
        var definition = [
          '#%RAML 0.8',
          '---',
      ...
      it('should succeed when dealing with URI parameters with two types', function(done) {
        ...
        //FIXTEST file can not be used here
        raml.load(definition).should.become({
          title: 'Test',
          baseUri: 'http://{a}.myapi.org',
          baseUriParameters: {
...
//FIXTEST file type can not be used here
        raml.load(definition).should.become({
          title: 'Test',
          baseUri: 'http://{a}.myapi.org',
          resources: [
...
//FIXTEST file parameter is not allowed here
      it('should succeed when dealing with URI parameters with two types', function(done) {
        var definition = [
          '#%RAML 0.8',
          '---',
...
//FIXTEST one type should be allowed
      it('should pass when a parameter uses array syntax with only one type', function(done) {
        var definition = [
          '#%RAML 0.8',
          '---',
...
//
    ...
    //});
//FIXTEST query parameters
//    describe('Named parameters in query string parameter', function(){
      it('should succeed with null query parameters', function(done) {
        var definition = [
          '#%RAML 0.8',
          '---',
          'title: Test',
...
//FIXTEST description
      it('should fail when a parameter uses array syntax with no type', function(done) {
        var definition = [
          '#%RAML 0.8',
          '---',
          'title: Test',
...
//    describe('Named parameters in form parameters', function(){
      it('should fail if formParameters is used in a response', function(done){
        raml.load([
          ...
          '            formParameters:',
          '                a:'
        ].join('\n')).should.be.rejectedWith(/Form parameters can not be used in response/).and.notify(done);
      });
//
      it('should fail if formParameters is used together with schema', function(done){
        raml.load([
            ...
            '      application/json:',
            '        formParameters:',
            '        schema: s',//to get correct order of messages
            ''
        ].join('\n')).should.be.rejectedWith(/'formParameters' property cannot be used together with the 'example' or 'schema' properties/).and.notify(done);
...
//FIXTEST no default media type
      it('should fail when a parameter uses array syntax with only no type', function(done) {
        var definition = [
          '#%RAML 0.8',
          '---',
          'title: Test',
...
////  describe('Resources', function() {
    it('should fail on duplicate absolute URIs', function(done) {
      ...
      raml.load(definition).should.be.rejectedWith(/Resources share same URI/).and.notify(done);
    });
...
//TESTFIX ~ is not defined in 0.8 spec
    it('should fail when a method is scalar', function(done) {
      var definition = [
          '#%RAML 0.8',
          '---',
          'title: Test',
...
// TESTFIX (error message in fact value is a scalar)
    it('is should fail when resource is a scalar', function(done) {
      ...
      raml.load(definition).should.be.rejectedWith(/Property '\/' can not have scalar value/).and.notify(done);
    });
//TESTFIX (it vas a scalar in /:)
...
//  describe('Resource Responses', function() {
    //TESTIFIX test description
    it('should fail with arrays as keys', function(done) {
      var definition = [
        '#%RAML 0.8',
        '---',
        'title: Test',
...
//
    it('should fail to load a yaml with hash as key', function(done) {
      ...
      raml.load(definition).should.be.rejectedWith(/Node key can not be a map/).and.notify(done);
    });
////  });
////
////  describe('Traits at resource level', function() {
//
//    //FIXTEST (it should be discussed but as for me there is no reason why display name is not inherited) @Pavel
    // I also see no reason for displayName to be ignored, thus the test remain unchanged, instead I added displayName to the other
    // tests, which were misssing it
...
////FIXTEST it is modelled after example in RAML 0.8 spec which looks incorrect, I was not able to find any wording that this is allowed
...
////
    it('should succeed if trait is missing displayName property', function(done) {
      var definition = [
        ...
        '      responses:',
        '        503:',
        '          description: Server <<parameter>> Unavailable. Check Your Rate Limits.',
        '/:',
...
////FIXTEST Changed description
    it('should fail if traits value is scalar', function(done) {
      ...
      raml.load(definition).should.be.rejectedWith(/Property 'traits' can not have scalar value/).and.notify(done);
    });
//
    it('should fail if traits value is dictionary', function(done) {
      ...
      raml.load(definition).should.be.rejectedWith(/Node: 'trait1' should be wrapped in sequence/).and.notify(done);
    });
//FIXME
    ...
    it('should fail on invalid trait name', function(done) {
      ...
      raml.load(definition).should.be.rejectedWith('Unrecognized trait: \'throttled\'.').and.notify(done);
    });
//
...
////
    ...
    // Note that there exists the same test where trait declares the parameters and they, thus, are not
    // considered unused)
    it('should reject unused parameter whose value is a map', function(done) {
      var definition = [
        '#%RAML 0.8',
        '---',
        'title: Test',
      ...
      raml.load(definition).should.be.rejectedWith(/Unused parameter: 'param1'/).and.notify(done);
    });
////
    it('should reject trait with missing provided parameters', function(done) {
      ...
      raml.load(definition).should.be.rejectedWith(/Value is not provided for parameter: 'lalalalala'/).and.notify(done);
    });
////
    ...
    //FIXTEST several other tests including "traits across !include boundaries" do have displayName being applied by the trait
    //There is nothing in the specs making "displayName" to be ommitted during trait application, thus this test should also
    //have display name in JSON
...
//  describe('Traits at method level', function() {
    ...
    //FIXTEST as with other similar cases, there is nothing in the spec forbidding the application of displayName from traits,
    //so adding it to the test.
    it('should remove nodes with question mark that are not used', function(done) {
      var definition = [
        '#%RAML 0.8',
        '---',
    ...
    it('should fail if use property is not an array 2', function(done) {
      var definition = [
        '#%RAML 0.8',
        '---',
        'title: Test',
    ...
    //FIXTEST changed message name for an equal one from our parser.
    it('should fail on invalid trait name 2', function(done) {
      var definition = [
        '#%RAML 0.8',
        '---',
        'title: Test',
    ...
    //FIXTEST consistent behaviour with another example object structure should not depend from original YAML syntax
    //FIXTEST several other tests including "traits across !include boundaries" do have displayName being applied by the trait
    //There is nothing in the specs making "displayName" to be ommitted during trait application, thus this test should also
    //have display name in JSON
    it('should allow dictionary keys as names of traits', function(done) {
...
//
    ...
    it('should reject parameters whose value is an array', function(done) {
      var definition = [
        ...
        'traits:',
        '  - rateLimited:',
        '      displayName: Rate Limited<<param1>>',
        '      headers?:',
        '        If-None-Match?:',
        '          description: |',
        '            If-None-Match headers ensure that you don’t retrieve unnecessary data',
...
//  describe('Resource Types', function () {
    ...
    it('should fail if resourceTypes value is scalar', function(done) {
      ...
      raml.load(definition).should.be.rejectedWith(/Property 'resourceTypes' can not have scalar value/).and.notify(done);
    });
//
    it('should fail if resourceTypes value is dictionary', function(done) {
      ...
      raml.load(definition).should.be.rejectedWith(/Node: 'type1' should be wrapped in sequence/).and.notify(done);
    });
//
    it('should fail if type is an array', function(done) {
      ...
      raml.load(definition).should.be.rejectedWith(/Unrecognized resource type: \'foo\'./).and.notify(done);
    });
//
    it('should fail if resource is of a missing type', function(done) {
      ...
      raml.load(definition).should.be.rejectedWith(/Unrecognized resource type: \'invalidType\'./).and.notify(done);
    });
//
    ...
    it('should fail if resource type is null', function(done) {
      var definition = [
        ...
        raml.load(definition).should.be.rejectedWith(/Resource type can not be null/).and.notify(done);
    });
//
    it('should fail if resource type is null', function(done) {
      var definition = [
...
//
    ...
    it('should fail if a resource type inherits from a missing type', function(done) {
      ...
      raml.load(definition).should.be.rejectedWith(/Unrecognized resource type: \'missing\'./).and.notify(done);
    });
//
    it('should fail if a resource type applies a missing trait', function(done) {
      ...
      raml.load(definition).should.be.rejectedWith('Unrecognized trait: \'bar\'.').and.notify(done);
    });
//
...
//
    ...
    //FIXTEST
    //There is nothing in the specs making "displayName" to be ommitted during resource type application, thus this test should also
    //have display name in JSON
    ...
    //FIXTEST added displayName as nothing in the spec states that displayName should not be applied on Resource type application.
    //Also changed the order of methods just so the test pass, there is nothing in the specs, which defines, which order is correct in this case
    ...
    it('should fail if parameters are missing', function(done) {
      ...
      raml.load(definition).should.be.rejectedWith(/Value is not provided for parameter: 'bar'/).and.notify(done);
    });
//
    it('should fail if resourceType uses a missing trait', function(done) {
      ...
      raml.load(definition).should.be.rejectedWith('Unrecognized trait: \'blah\'').and.notify(done);
    });
//

    //FIXTEST added displayName as there is nothign in the spec forbidding displayName application on resource type or trait merging.
    //Also added "is": ["secured"] to the resource for the same reasons.
...
//
    ...
    it('should fail when specifying schemas is scalar', function(done) {
      ...
      raml.load(definition).should.be.rejectedWith(/Property 'schemas' can not have scalar value/).and.notify(done);
    });
//
    //FIXTEST while this one is correct, its importance looks quite low for me, nobody is going to put in an empty map in real life.
    //thus added a test where there is something inside the map below instead. Lets pass this one with a low priority later.
//    it('should fail when specifying schemas is scalar', function(done) {
//        var definition = [
//            '#%RAML 0.8',
//            '---',
...
//    });
    ...
    it('should fail when specifying schemas is a map', function(done) {
      var definition = [
        '#%RAML 0.8',
        '---',
        'title: Test',
      ...
      raml.load(definition).should.be.rejectedWith(/Node: 'testSchema' should be wrapped in sequence/).and.notify(done);
    });
//
...
//
    it('should fail if a schema is a map', function(done) {
      ...
      raml.load(definition).should.be.rejectedWith(/Can not parse schema/).notify(done);
    });
//
...
//
    ...
    //FIXME The test looks invalid. Schemas foo and faa are not valid. "-" is missed before faa. foo2 and foo3 references can not be resolved.
    //And, finally, there's nothing about a trait in this test contrary to the test name. All in all, suggest excluding from the test suite.
...
//    });
//
    //FIXME As with the test above, the test looks invalid. Schema foo is not valid. foo2 reference can not be resolved.
    //And, finally, there's nothing about a trait in this test contrary to the test name. All in all, suggest excluding from the test suite.
...
//  describe('Security schemes', function(){
    it('should fail when schemes is map', function(done) {
      ...
      raml.load(definition).should.be.rejectedWith(/Node: 'foo' should be wrapped in sequence/).and.notify(done);
    });
//
    it('should fail when schemes is scalar', function(done) {
      ...
      raml.load(definition).should.be.rejectedWith(/Property 'securitySchemes' can not have scalar value/).and.notify(done);
    });

    //FIXTEST to be discussed, but I see no reason why empty securitySchemes is something wrong.
...
//
    //FIXTEST pretty strange syntax. Not sure what it was supposed to mean.
...
//
    it('should fail when scheme is scalar', function(done) {
      ...
      raml.load(definition).should.be.rejectedWith(/Node 'scheme' can not be a scalar/).and.notify(done);
    });
//
...
//
    it('should fail when scheme contains a wrong property', function(done) {
      ...
      raml.load(definition).should.be.rejectedWith(/Unknown node: 'property'/).and.notify(done);
    });
//
    it('should fail when scheme does not have type', function(done) {
      ...
      raml.load(definition).should.be.rejectedWith(/Unrecognized security scheme type: \'ss\'. Allowed values are: 'OAuth 1.0', 'OAuth 2.0', 'Basic Authentication', 'DigestSecurityScheme Authentication', 'x-\{other\}'/).and.notify(done);
    });
//
...
//
    it('should fail when declaring a URI parameter in a resource with a wrong property', function(done) {
      ...
      raml.load(definition).should.be.rejectedWith(/Unknown node: 'blah'/).and.notify(done);
    });
//
    ...
    //FIXTEST changed message from /Unused url parameter \'not-used\'/
...
//
    it('should fail if headers is string', function(done) {
      ...
      raml.load(definition).should.be.rejectedWith(/Property 'headers' can not have scalar value/).and.notify(done);
    });
//
...
//
    it('should fail if header is scalar', function(done) {
      ...
      raml.load(definition).should.be.rejectedWith(/Node 'foo' can not be a scalar/).and.notify(done);
    });
//
...
//
    it('should fail if header uses unknown property', function(done) {
      ...
      raml.load(definition).should.be.rejectedWith(/Unknown node: 'foo'/).and.notify(done);
    });
//
    it('should fail if queryParams is string', function(done) {
      ...
      raml.load(definition).should.be.rejectedWith(/Property 'queryParameters' can not have scalar value/).and.notify(done);
    });
//
...
//
    it('should fail if body is a scalar', function(done) {
      ...
      raml.load(definition).should.be.rejectedWith(/Property 'body' can not have scalar value/).and.notify(done);
    });
//
...
//
    it('should fail if body is using implicit after explicit body', function(done) {
      ...
      raml.load(definition).should.be.rejectedWith(/invalid media type/).and.notify(done);
    });
//
...
//
    ...
    it('should fail if queryParameters is array', function(done) {
      ...
      raml.load(definition).should.be.rejectedWith(/node should have at least one member value/).and.notify(done);
    });
//
    it('should succeed if queryParameters is null', function(done) {
      var definition = [
...
//
    it('should fail if responses is scalar', function(done) {
      ...
      raml.load(definition).should.be.rejectedWith(/Property 'responses' can not have scalar value/).and.notify(done);
    });
//
...
//  describe('Base Uri Parameters', function(){
    ...
    //FIXTEST changed message from /Unused url parameter \'domainName\'/
...
//
    it('should fail if has null content', function(done) {
      ...
      raml.load(definition).should.be.rejectedWith(/Property \'content\' must be a string/).and.notify(done);
    });
//  });
//
//  describe('Default Media Type', function() {
    it('should fail if mediaType property is null', function(done) {
      ...
      raml.load(definition).should.be.rejectedWith(/Property \'mediaType\' must be a string/).and.notify(done);
    });
//
...
//
    ...
    //FIXTEST (inserted into 'expected' inherited 'is' for method)
...
//
      //FIXTEST replaced schema/examples with simpliest passing validation. Added is: [ "bodiable" ] to a resource.
...
//
    //FIXTEST added is: ["bodiable"] to a method
      it('should apply mediaType property in a trait composed with a resourceType', function(done) {
        var definition = [
          '#%RAML 0.8',
          '---',
...
//
      //FIXTEST added is: ["bodiable"] to a secondLevel resource because its actually bodiable.
      it('should apply mediaType property in a trait composed with a resourceType which inherits from another RT and applies a trait', function(done) {
        var definition = [
          '#%RAML 0.8',
          '---',
...
//  describe('Error reporting', function () {
    ...
    //FIXTEST not sure what this test tries to achieve, the error is certainly going to be regarding missing title.
    //Added a test actually checking for missing title error message below
//    it('should report correct line/column for missing title', function(done) {
//      var noop = function () {};
//      var definition = [
//        '#%RAML 0.8',
...
//
    //FIXTEST changed "7" position in line 2 to "0" as we always blame the property, not its value.
...
//      var file = path.join(__dirname, '../raml-files/RT-261.raml');

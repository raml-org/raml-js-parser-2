...
describe('Validator', function () {
  ...
  it('should fail if baseUriParameters is defined without baseUri', function (done) {
    raml.load([
      ...
      'baseUriParameters:',
      '  param1:',
    ].join('\n')).should.be.rejectedWith('Base uri parameter unused').and.notify(done);
  });
  ...
  //FIXTEST message changed from 'only HTTP and HTTPS values are allowed'
  ...
  //FIXTEST changed message from 'only HTTP and HTTPS values are allowed'
  ...
  //FIXTEST Message changed from 'property: \'protocols\' is invalid in a resource'
  ...
  //baseUri is not a required property
  ...
  //FIXTEST chanfwd message from 'OAuth 2.0 settings must have accessTokenUri property'
  ...
  //FIXTEST chanfwd message from 'OAuth 2.0 settings must have authorizationUri property'
  ...
  //FIXTEST changed message from 'OAuth 1.0 settings must have tokenCredentialsUri property'
      ...
      [
        ...
        it('should reject because number|integer property is unusable for the type ' + type, function (done) {
          raml.load([
            ...
            '      type: ' + type,
            '      ' + property[0] + ': ' + property[1]
          ].join('\n')).should.be.rejectedWith('Property \'' + property[0] + '\' can only be used if type is \'integer\' or \'number\'').and.notify(done);
        });
      });
    });
  })();

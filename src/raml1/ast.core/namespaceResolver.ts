...
export class NamespaceResolver{
                ...
                if(x.parent()==null) {
                    ...
                    if(map) {
                        for (var absPath of Object.keys(map)) {
                            ...
                            if (path.isAbsolute(info.includePath) || jsyaml.isWebPath(info.includePath)) {
                                includePath = childInfo.absolutePath();
                            }
                            else if (path.isAbsolute(childInclude) || jsyaml.isWebPath(childInclude)) {
                                includePath = childInfo.absolutePath();
                            }
                            else {
    ...
    private calculateNamespaces(unit:ll.ICompilationUnit):{[key:string]:UsesInfo}{
        ...
        for (var un of usesDeclarationNodes) {
            ...
            var includePath;
            if (path.isAbsolute(value) || jsyaml.isWebPath(value)) {
